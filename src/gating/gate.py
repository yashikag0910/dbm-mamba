...
class AdaptiveInferenceGate(nn.Module):
    ...
    def __init__(self, entropy_threshold: float = 1.2, var_iat_threshold: float = 0.5,
                 mamba_confidence_th: float = 0.95, anomaly_confidence_th: float = 0.5):
        super().__init__()
        ...
        self.anomaly_confidence_th = anomaly_confidence_th
    ...
    def check_tcp_escalation(self, seq_x: torch.Tensor) -> torch.Tensor:
        """
        TCP escalation rule:
        Escalate immediately to Mamba if any packet has high flags (TCP SYN/RST scan signs).
        In loaders, flags feature is flags_int / 32.0.
        Let's trigger escalation if flags feature > 0.5 (representing SYN, RST, or FIN scan attempts).
        """
        flags = seq_x[:, :, 3] # (B, L)
        # Check if any packet has flags > 0.5 (which corresponds to SYN, SYN-ACK, RST, etc.)
        escalate = torch.any(flags > 0.5, dim=1) # (B,)
        return escalate
    ...
    def self_distill_labels(self, seq_x: torch.Tensor, mamba_logits: torch.Tensor, 
        ...
        anomaly_cond = anomaly_scores < (self.anomaly_confidence_th * tau_d)
        
        low_complexity_pref = confidence_cond & anomaly_cond
        
        # Targets: 1.0 for high complexity, 0.0 for low complexity
